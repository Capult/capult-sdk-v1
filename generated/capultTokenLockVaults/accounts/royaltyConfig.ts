...
export function findRoyaltyConfigPda(
  ...
  return context.eddsa.findPda(programId, [
    ...
    publicKeySerializer().serialize(seeds.royaltyInitKey),
    bytes().serialize(new Uint8Array([82, 79, 89, 65, 76, 84, 89, 95, 67, 79, 78, 70, 73, 71, 95, 83, 69, 69, 68])),
  ]);
}

export async function fetchRoyaltyConfigFromSeeds(
  context: Pick<Context, 'eddsa' | 'programs' | 'rpc'>,
  seeds: Parameters<typeof findRoyaltyConfigPda>[1],
  options?: RpcGetAccountOptions
): Promise<RoyaltyConfig> {
  return fetchRoyaltyConfig(context, findRoyaltyConfigPda(context, seeds), options);
}

export async function safeFetchRoyaltyConfigFromSeeds(
  context: Pick<Context, 'eddsa' | 'programs' | 'rpc'>,
  seeds: Parameters<typeof findRoyaltyConfigPda>[1],
  options?: RpcGetAccountOptions
): Promise<RoyaltyConfig | null> {
  return safeFetchRoyaltyConfig(context, findRoyaltyConfigPda(context, seeds), options);
}
